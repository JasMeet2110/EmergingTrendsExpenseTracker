...
const palette = [
  "#2563EB", "#F59E0B", "#EF4444", "#8B5CF6", "#EC4899",
  "#06B6D4", "#22C55E", "#F97316", "#A855F7", "#14B8A6",
  "#EAB308", "#F43F5E", "#3B82F6", "#84CC16", "#0EA5E9",
];
...
function arcPath(cx: number, cy: number, rOuter: number, rInner: number, start: number, end: number) {
  ...
  const sxO = cx + rOuter * Math.cos(start),
    syO = cy + rOuter * Math.sin(start);
  const exO = cx + rOuter * Math.cos(end),
    eyO = cy + rOuter * Math.sin(end);
  const sxI = cx + rInner * Math.cos(end),
    syI = cy + rInner * Math.sin(end);
  const exI = cx + rInner * Math.cos(start),
    eyI = cy + rInner * Math.sin(start);
  return [
    `M ${sxO} ${syO}`,
...
export default function PieChartScreen() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // filter txns within selected month
        ...
        <>
          <View style={styles.chartWrap}>
            ...
            <View style={styles.centerLabel}>
              <Text style={styles.centerTop}>
                {remainingNow > 0 ? "Remaining" : "Expenses"}
              </Text>
              <Text
                style={[
                  styles.centerVal,
                  remainingNow > 0 ? styles.pos : styles.neg,
                ]}
              >
          ...
          {overBudget ? (
            <Text style={styles.note}>
              Expenses exceed income — pie shows expenses only.
            </Text>
          ) : null}
            ...
            <FlatList
              ...
              renderItem={({ item, index }) => (
                <View style={styles.tr}>
                  ...
                  <Text
                    style={[styles.tdText, { flex: 1, textAlign: "right" }]}
                  >
                    {fmtMoney(item.amount)}
                  </Text>
                  <Text
                    style={[styles.tdText, { width: 70, textAlign: "right" }]}
                  >
